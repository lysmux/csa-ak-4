...
Умножение и деление значений `long` не поддерживаются. При попытке
использовать 64-битные операнды в `*` или `/` анализатор выдаёт
...
Полный цикл инструкции = `FETCH` + `EXECUTE` (N шагов) + `CHECK_IRQ` = **N + 2 такта**.
Такт `CHECK_IRQ` выполняется только при `IE = 1`; при запрещённых прерываниях
(в том числе внутри обработчика) инструкция завершается переходом сразу в `FETCH`
и занимает **N + 1 такт**. Для `HALT` последний такт переводит процессор в
состояние `HALT`.

| Такты | Группа              | Инструкции                                             |
|:-----:|---------------------|--------------------------------------------------------|
| **1** | Базовые             | `NOP`, `HALT`                                          |
| **1** | Стек                | `PUSH`, `DUP`, `SWAP`, `OVER`                          |
| **1** | Арифметика и логика | `CMP`, `I2L`, `INC`, `DEC`, `NEG`, `NOT`, `SHL`, `SHR` |
| **1** | Управление          | `CALL`, `JMP`, условные переходы                       |
| **1** | Прерывания          | `EI`, `DI`                                             |
| **2** | Память              | `LOAD`, `LOADI`                                        |
| **2** | Стек и возврат      | `RET`, `DROP`, `PSHR`, `POPR`                          |
| **2** | Арифметика и логика | `ADD`, `SUB`, `MUL`, `DIV`, `ADDC`, `AND`, `OR`, `XOR` |
| **3** | Память              | `STORE`                                                |
| **3** | Управление          | `LOOP`                                                 |
| **4** | Память              | `STOREI`                                               |
| **4** | Прерывания          | `RTI`                                                  |
| **4** | Двойная точность    | `DADD`, `DSUB`                                         |
...
## Тестирование
- **Golden-тесты** (`tests/yaml/`): для каждого алгоритма фиксируются исходный код,
...
| `hello`             | 3   | 14                | 70               | 274       |
| `cat`               | 14  | 46                | 230              | 579       |
| `hello_user_name`   | 25  | 117               | 585              | 2548      |
| `sort`              | 39  | 193               | 965              | 4354      |
| `long_arithmetic`   | 6   | 26                | 130              | 81        |
| `palindrome`        | 42  | 193               | 965              | 2 145 915 |
| `algorithms`        | 25  | 103               | 515              | 804       |
| `interrupt_overrun` | 14  | 46                | 230              | 1645      |
