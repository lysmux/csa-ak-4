...
Полный цикл инструкции = `START` + `FETCH` + `EXECUTE` (N шагов) = **N + 2 такта**.
Для `HALT` последний такт переводит процессор в состояние `HALT`; остальные
инструкции завершаются переходом в `START`.

| Такты | Группа              | Инструкции                                             |
|:-----:|---------------------|--------------------------------------------------------|
| **3** | Базовые             | `NOP`, `HALT`                                          |
| **3** | Стек                | `PUSH`, `DUP`, `SWAP`, `OVER`                          |
| **3** | Арифметика и логика | `CMP`, `I2L`, `INC`, `DEC`, `NEG`, `NOT`, `SHL`, `SHR` |
| **3** | Управление          | `CALL`, `JMP`, условные переходы                       |
| **3** | Прерывания          | `EI`, `DI`                                             |
| **4** | Память              | `LOAD`, `LOADI`                                        |
| **4** | Стек и возврат      | `RET`, `DROP`, `PSHR`, `POPR`                          |
| **4** | Арифметика и логика | `ADD`, `SUB`, `MUL`, `DIV`, `ADDC`, `AND`, `OR`, `XOR` |
| **5** | Память и управление | `STORE`, `LOOP`                                        |
| **6** | Память и прерывания | `STOREI`, `RTI`                                        |
| **6** | Двойная точность    | `DADD`, `DSUB`                                         |
